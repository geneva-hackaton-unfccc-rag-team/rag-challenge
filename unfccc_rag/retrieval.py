...
from unfccc_rag.utils import load_pickled_chunks, minmax_normalize

logger = logging.getLogger(__name__)


def load_retrieval_engine(
    embeddings_file: Path, model_name: str = "google/embeddinggemma-300m"
) -> Callable[[str], str]:
    """Load the retrieval engine.

    Args:
        embeddings_file: The path to the embeddings file.
        model_name: The name of the embedding model to use.

    Returns:
        A callable that takes a query and returns a string containing the document chunks and their respective proximity scores.
    """
    ...
    chunks = load_pickled_chunks(embeddings_file)
